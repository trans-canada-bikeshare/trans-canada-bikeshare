...
# Only the timestamp line is excluded. The prefix alone would also match the
# static "Generated by `pipeline/quality_report.py`..." sentence, which made
# that sentence editable without failing the gate — demonstrated in review.
GENERATED_LINE = re.compile(
    rf"^{re.escape(quality_report.GENERATED_PREFIX)}\d{{4}}-\d{{2}}-\d{{2}}T"
)
